...
(function($, e, b) {
  ...
  f = (function() {
    ...
    navigator.userAgent.match(/msie [6]/i) && !d && (function() {
      var q, r;
      j.start = function() {
        if (!q) {
          r = $.fn[c].src;
...
/* 
 * Extending the native Array prototype for clean empty (or other) values.
 */
...
/* 
 * Helper function for find language in hash.
 */
function check_current_site_language_from_hash() {
  // Try to find language in hash.
  current_site_language = 'default';
  hash = window.location.hash;
  hash_array = hash.replace('#!', '').split('/').clean('');
  if (hash_array.length > 0) {
    if ($.inArray(hash_array[0], $.site_languages) > -1) {
      current_site_language = hash_array[0];
    }
  }
...
/* 
 * Helper function for find language in path.
 */
function check_current_site_language_from_path() {
  // Try to find language in hash.
  current_site_language = 'default';
...
(function($) {
  if (!$.hash)
    $.hash = '#!/';
  if (!$.siteurl)
    $.siteurl = document.location.protocol + '//' + document.location.host; // your site url
  if (!$.container)
    $.container = '#content'; // container SELECTOR to use for hash navigation
  if (!$.site_languages)
    $.site_languages = undefined;
  if (!$.pjax_timeout)
    $.pjax_timeout = 0;
  ...
  $.fn.pjaxform = function(selector, container, options) {
    if (options)
      options.container = container;
    else
      options = $.isPlainObject(container) ? container : {
    ...
    return $(this).on('click', selector, function(event) {
      $(options.container).trigger('form-submit.pjax', $(event.currentTarget));
      ...
      var defaults = {
        url: (options.type && options.type.toUpperCase() == 'GET') ? event.currentTarget.action + '?' + data : event.currentTarget.action,
  ...
  var pjax = $.pjax = function(options) {
    var $container = $(options.container),
      success = options.success || $.noop, $settings = [];
    ...
    options.success = function(data, textStatus, jqXHR) {
      ...
      // Make it happen.
      //set timeout before success for animation
      var _this = this;
      ...
      setTimeout(function() {
        ...
        // the page's title.
        var title = $.trim(_this.find('div#new-doc-title').remove().text());
        if (title)
      ...
      if ($.support.pjax)
      {
        ...
        if (!options.isform || options.type.toUpperCase() == 'GET') {
          location_hash = "!" + options.url.replace(options.siteurl, "");
          // Remove language from hash.
          current_site_language = check_current_site_language_from_path();
          if (current_site_language != 'default') {
            location_hash = location_hash.replace(current_site_language + '/', '');
          }
  ...
  else if (location.pathname.length > 1 || location.search.length > 1)
  {
    if (!$.support.pjax) {
      ...
      location_pathname = window.location.pathname;
      if (current_site_language != 'default') {
        current_site_language = '/' + current_site_language;
        location_pathname = window.location.pathname.replace(current_site_language, '');
      } else {
        current_site_language = '';
      }
      window.location = $.siteurl + current_site_language + '/#!' + location_pathname + window.location.search;
    }
  }
      ...
      if ((hash.substr(0, 2) == '#!' || hash == '') && hash != $.hash) {
        ...
        $.ajax({
          ...
          beforeSend: function(jqXHR, settings) {
            jqXHR.setRequestHeader('X-PJAX', 'true');
            jqXHR.setRequestHeader('X-PJAX-SUPPORT', $.support.pjax ? true : false);
            jqXHR.setRequestHeader('X-PJAX-SITE-LANGUAGE', current_site_language);
          ...
          success: function(data, textStatus, jqXHR) {
            ...
            setTimeout(function() {
              $($.container).html(data);
              // If there is no pjax support, we should handle title changes
              // If there's a <title> tag in the response, use it as
              // the page's title.
              var title = $.trim($($.container).find('div#new-doc-title').remove().text());
              if (title)
                document.title = title
            }, $.pjax_timeout);
            //$($.container).html(data);
